...
export class EventTarget {
	...
	addEventListener(type, callback, options) {
		...
		for (const l of this[_listeners]) {
			if (
				l.type === listener.type && l.callback === listener.callback &&
	...
	dispatchEvent(event) {
		if (event[_dispatchFlag]) {
			...
			throw new Error(
				"InvalidStateError: Trying to dispatch an event while it's already being dispatched",
			);
		}
...
function invoke(event, target, capture) {
	...
	for (const listener of target[_listeners]) {
		if (
			event.type !== listener.type || listener.capture !== capture ||
			listener.removed
		) {
			continue;
		}
